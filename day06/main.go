...
func main() {
	fmt.Println(toggleLights(input))     // 400410
	fmt.Println(toggleBrightness(input)) // 15343601
}

func toggleBrightness(input string) int {
	instrs := parse(input)
	grid := make(map[point]int)
	for _, instr := range instrs {
		for y := instr.from.y; y <= instr.to.y; y++ {
			for x := instr.from.x; x <= instr.to.x; x++ {
				if instr.on {
					grid[point{x, y}]++
				} else if instr.off {
					grid[point{x, y}] = max(0, grid[point{x, y}]-1)
				} else {
					grid[point{x, y}] += 2
				}
			}
