...
func main() {
	prim := parse(primary)
	fmt.Println("part 1:", part1(prim, input)) // 373
	fmt.Println("part 2:", part2(prim, input)) // 260
}

func part1(prim map[string]string, input string) int {
	lines := strings.Split(input, "\n")
	for sue, line := range lines {
		i := strings.Index(line, ":")
		kvs := strings.Split(strings.TrimSpace(line[i+1:]), ",")
...
func part2(prim map[string]string, input string) int {
	lines := strings.Split(input, "\n")
	for sue, line := range lines {
		i := strings.Index(line, ":")
		kvs := strings.Split(strings.TrimSpace(line[i+1:]), ",")
